...
cdf() {
  local dir
  dir=$(find . -maxdepth 3 -type d -not -path '*/\.*' | fzf --preview 'ls -l {} | batcat --style=numbers --color=always') && cd "$dir"
}

# Fuzzy find directories in ~/projects and ~/work with preview
ff() {
  local dir
  dir=$(find ~/projects ~/work /mnt/c/dev ~/.config -maxdepth 2 -type d -not -path '.*' | fzf --preview 'ls -l {} | bat --style=numbers --color=always') && cd "$dir"
}
